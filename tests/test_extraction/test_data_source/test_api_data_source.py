...
class TestAPIDataSource:
    ...
    def setup(self, mocker):
        ...
        self.data_source = APIDataSource(
            self.url, self.default_params, self.mock_validator
        )
    ...
    def mock_request(self, mocker):
        def response_factory(status_code=200):
            ...
            response_properties = {
                ...
                "json.return_value": {"res": "test"},
            }
            mock_response.configure_mock(**response_properties)
            self.get = mocker.patch(
                "extraction.source.api_data_source.get", return_value=mock_response
            )
