...
class TestPipeline:
    ...
    def mock_resolve_path(self, mocker: MockerFixture):
        def resolve_path_mock(filename):
            mocked_resolve_path = mocker.patch(
                "process.pipeline.resolve_path", return_value=Path(filename)
            )
    ...
    def mock_parse_steps(self, mocker: MockerFixture):
        def parse_steps_mock(steps):
            mocked_parse_steps = mocker.patch(
                "process.pipeline.parse_steps", return_value=steps
            )
    ...
    def mock_open_file(self, mocker: MockerFixture):
        def open_file_mock(functions=None):
            ...
            mocked_safe_open = mocker.patch(
                "process.pipeline.yaml.safe_load",
                return_value=[{"run": "test", "name": "test"}],
            )
    ...
    def test_success_str_initialization(
        self, mock_resolve_path, mock_parse_steps, mock_open_file
    ):
